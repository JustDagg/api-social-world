...
exports.postById = (req, res, next, id) => {
    Post.findById(id)
        ...
        .populate('postedBy', '_id name')
        .select('_id title category location body created likes comments photo')
        .exec((err, post) => {
            if (err || !post) {
...
exports.getPosts = (req, res) => {
    ...
    const posts = Post.find()
        ...
        .populate('comments.postedBy', '_id name')
        .select("_id title category location body photo created likes")
        .sort({ created: -1 })
        .then((posts) => {
            res.json(posts);
        })
...
exports.getAllPostsRn = (req, res) => {
    const posts = Post.find()
        ...
        .select('_id title category location body created likes comments updated')
        .sort({ created: -1 })
        .then((posts) => {
            res.json(posts);
        })
...
exports.postsByUser = (req, res) => {
    Post.find({ postedBy: req.profile._id })
        ...
        .populate('comments.postedBy', '_id name')
        .populate('postedBy', '_id name')
        .select('_id title category location body photo created likes comments updated')
        .sort({ created: -1 })
        .exec((err, posts) => {
            if (err) {
                return res.status(400).json({
